 ...
 * </ul>
 * <p>
 * LogFile实现了SimpleDb的恢复子系统。此类能够根据需要编写不同的日志记录，但调用者有责任确保遵循提前写入日志和两阶段锁定规则。  <p>
 *
 * <u> 锁定说明 </u>
 * <p>
 * <p>
 * 这里的许多方法都是同步的（以防止并发日志写入发生）; BufferPool中的许多方法也是同步的（出于类似的原因。）问题是BufferPool写日志记录（页面刷新），日志文件刷新BufferPool
 * 页面（检查点和恢复）。这可能导致死锁。因此，任何需要访问BufferPool的LogFile操作都不能声明为synchronized，并且必须以如下块开头：
 *
 * <p>
 * <pre>
 * synchronized (Database.getBufferPool()) {
 ...
 * <ul>
 *
 * <li> 文件的第一个长整数表示最后写入的检查点的偏移量，如果没有检查点，则为-1
 *
 * <li> 日志中的所有其他数据都包含日志记录。日志记录是可变长度的。
 *
 * <li> 每个日志记录以整数类型和长整数事务ID开头。
 *
 * <li> 每个日志记录以一个长整数文件偏移结束，表示记录开始的日志文件中的位置。
 *
 * <li> 有五种记录类型：ABORT，COMMIT，UPDATE，BEGIN和CHECKPOINT
 *
 * <li> ABORT，COMMIT和BEGIN记录不包含其他数据
 *
 * <li> UPDATE RECORDS由两个条目组成，即前映像和后映像。这些图像是序列化的Page对象，可以使用LogFile.readPageData（）和LogFile
 * .writePageData（）方法进行访问。有关示例，请参见LogFile.print（）。
 *
 * <li> CHECKPOINT记录由检查点获取时的活动事务和它们在磁盘上的第一个日志记录组成。记录的格式是事务数的整数计数，以及每个活动事务的长整数事务id和长整数第一个记录偏移量。
 *
 * </ul>
 */
...
public class LogFile {
    ...
    Page readPageData(RandomAccessFile raf) throws IOException {
        ...
        try {
            ...
            //            Debug.log("READ PAGE OF TYPE " + pageClassName + ", table = " + newPage.getId().getTableId
            // () + ", page = " + newPage.getId().pageno());
